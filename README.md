...
double-uncertainty view — bupivacaine 100 mg at lumbar_epidural (tier C):
  predicted peak: total 1.099 ug/mL  ·  free 0.0549 ug/mL  ·  Tmax 34 min
...
│   ├── la.py                    # LA site-absorption PK (Bateman) + site dominance (LA0) + free conc. & double-uncertainty view (LA1) + cardiotoxicity comparison (LA2)
...
| **v0.6 — Local anesthetics (LAST)** | site-driven absorption; toxicity-threshold *ranges*; stereochemistry/cardiotoxicity ([spec](docs/specs/v0.6/local_anesthetics.md)) | 🟢 **LA0 + LA1 + LA2 shipped** — the `local_anesthetics` subsystem: one-compartment disposition + site-specific absorption (rank robust, `ka` Tier-C) + binding (LA0); the `toxicity_threshold` **ranges** (never lines), the **double-uncertainty view** (`la.double_uncertainty`, `hypnos la --site`), the `la.free_concentration` trace, range-not-line + saturable→caveat enforcement, the LA verification checklist, and `hypnos:safetyCritical` + threshold-range RDF in every export (Knudsen 1997 / Scott 1989; LA1); the drug-level `cardiotoxicity_class` + the agent-choice axis (`la.cardiotoxicity_comparison`, `hypnos la --agents`), a new **levobupivacaine** model (Bardsley 1998), and the numeric CNS-to-CVS fold-margin monotone with the ranking (LA2). Lidocaine/bupivacaine/levobupivacaine/ropivacaine. **Next:** the nonlinear binding-saturation free-fraction model (LA3) — a safety-framing step, not a dose calculator |
