...
│   ├── la.py                    # LA site-absorption PK (Bateman) + site dominance (LA0) + free conc. & double-uncertainty view (LA1)
...
| **v0.6 — Local anesthetics (LAST)** | site-driven absorption; toxicity-threshold *ranges*; stereochemistry/cardiotoxicity ([spec](docs/specs/v0.6/local_anesthetics.md)) | 🟢 **LA0 + LA1 shipped** — the `local_anesthetics` subsystem: one-compartment disposition + site-specific absorption (rank robust, `ka` Tier-C) + binding for lidocaine/bupivacaine/ropivacaine (Tucker & Mather 1979), the `hypnos.la` Bateman kernel + `hypnos la` site-dominance view (LA0); the `toxicity_threshold` **ranges** (never lines), the **double-uncertainty view** (`la.double_uncertainty`, `hypnos la --site`), the `la.free_concentration` trace, the schema range-not-line + saturable→caveat enforcement, the LA verification checklist, and the `hypnos:safetyCritical` + threshold-range RDF in every export (Knudsen 1997 / Scott 1989; LA1). **Next:** the bupivacaine-vs-ropivacaine cardiotoxicity comparison (LA2) and the nonlinear binding-saturation free-fraction model (LA3) — each a safety-framing step, not a dose calculator |
