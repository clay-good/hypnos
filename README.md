...
| Albumin | `--albumin <g/dL>` | albumin < 3.5 (hypoalbuminemia) | raised free fraction of highly-bound drugs (effect under-predicted) |
...
| **v0.5 — Breadth (reversal + special populations)** | reversal-agent kinetics; the organ-failure envelope + disease-state modifiers ([spec](docs/specs/v0.5/breadth.md)) | 🟢 **S0 shipped** — the **organ-function envelope** speaks: hepatic/renal/cardiac/albumin impairment greys every model with no cited standing (Tier-D + named extrapolation), remifentanil's esterase exception encoded (Dershwitz 1996 / Hoke 1997); live in `compare`, the CLI, and the dashboard. **Next:** the `reversal` subsystem (sugammadex/naloxone/flumazenil/neostigmine — B0/B1) and the opt-in, Tier-D `disease_state_modifier` records (S1) |
