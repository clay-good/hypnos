...
So the dataset reports its own status without flinching. Right now all fifteen models read `unverified`, and `hypnos status` will tell you exactly that: zero of fifteen verified, here are the highest-leverage ones to check first, here is the field-by-field checklist for each. A few models go further and refuse to run at all. The general-purpose Eleveld propofol model has an intricate maturation-and-allometry covariate structure that has not been transcribed and confirmed, so `simulate()` declines to evaluate it rather than risk shipping a mis-typed equation dressed up as a prediction. Refusing to compute is, occasionally, the most honest thing a tool can do.
