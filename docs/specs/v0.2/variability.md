...
> **Status:** **V0–V2 implemented; V3 in progress** (dataset/schema `0.2.0`). Extends [v0.1](../v0.1/spec.md) (which curates the fixed-effect / typical-value layer). Additive only — every v0.1 record stays valid; the variability block is optional and defaults to "not curated," which renders as an explicit gap, never as a fabricated band. Eleveld propofol now carries a verified-pathway (`unverified`) Ω-diagonal + Σ; `hypnos validate` enforces the §4 traps; `simulate`/`compare` draw seeded bands and the uncertainty-aware divergence; NONMEM and TCI-JSON exports carry the random-effects layer. See the roadmap (§12) for per-phase status and what remains (Schnider/opioid BSV backfill, PharmML/nlmixr2/Pumas random effects, off-diagonal `$OMEGA BLOCK`).
...
| Format | v0.1 behavior | v0.2 behavior |
| --- | --- | --- |
| **NONMEM** | `$OMEGA 0 FIX` / `$SIGMA 0 FIX` (nonmem.py §65-66 — "variability out of scope") | Emit the curated **`$OMEGA`** diagonal (and `$OMEGA BLOCK` when `omega_block` present) and **`$SIGMA`** from `residual_error` — *when the band tier permits*; otherwise keep `... 0 FIX` and emit a comment naming the missing component. The single most natural upgrade in the codebase. |
| **PharmML + SO** | typical-value `IndividualParameter`s | Random effects are **first-class** in PharmML: map η to `RandomEffect`/`VariabilityLevel` and ε to the observation error model. The durable interop anchor now carries the whole NLME object. |
| **nlmixr2 / rxode2**, **Pumas** | structural model | Add the `ini{}` / `@random` blocks (Ω) and the residual error model (Σ) — these ecosystems express NLME natively. |
...
| **V3 — Exports + breadth** | 🟡 partial | **done:** NONMEM `$OMEGA`/`$SIGMA` diagonal, TCI-JSON passthrough. **remaining:** `$OMEGA BLOCK`; PharmML/nlmixr2/Pumas random effects; backfill BSV for Schnider, the remifentanil trio, dexmedetomidine (the published Schnider CV%s in common toolchains look like RSEs, not BSV — backfill awaits source-table confirmation, per the never-invent rule). | The population model — not just its typical patient — round-trips through the pharmacometric formats. |
