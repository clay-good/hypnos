...
> **Status:** **V0–V3 implemented; the BSV data backfill is largely sourced — Eleveld (propofol), Minto + Kim (remifentanil), and Schnider (propofol, partial) now carry curated Ω diagonals (`pending_human_review`; Minto's from its OWN NONMEM $OMEGA output in the Open-TCI archive). Only Hannivoort dexmedetomidine BSV remains (primary table paywalled, honestly not_found).** (dataset/schema `0.2.0`). Extends [v0.1](../v0.1/spec.md) (which curates the fixed-effect / typical-value layer). Additive only — every v0.1 record stays valid; the variability block is optional and defaults to "not curated," which renders as an explicit gap, never as a fabricated band. Eleveld propofol now carries a verified-pathway (`unverified`) Ω-diagonal + Σ; `hypnos validate` enforces the §4 traps; `simulate`/`compare` draw seeded bands and the uncertainty-aware divergence; and **every export** carries the random-effects layer — NONMEM (`$OMEGA`/`$OMEGA BLOCK`/`$SIGMA`), PharmML (`VariabilityModel`), nlmixr2/rxode2 and Pumas (NLME companions), TCI-JSON (verbatim), and SBML (Ω/Σ as `hypnos:` RDF, since SBML core is deterministic) — including off-diagonal `$OMEGA BLOCK` where a complete correlated block is curated. See the roadmap (§12) for per-phase status and what remains (Schnider/opioid/dexmedetomidine BSV backfill — blocked on source-table confirmation, not on code).
...
| **SBML (L3v2)** | deterministic ODE | SBML core cannot express population random effects, so the typical-value model is emitted as today **plus** the curated Ω diagonal, Σ, and any off-diagonal correlations as MIRIAM/`hypnos:` RDF predicates inside the model annotation (`hypnos:betweenSubjectVariability` / `hypnos:residualError` / `hypnos:omegaCorrelation`), with an explicit SBML comment that a downstream deterministic consumer (COPASI/Tellurium) sees the typical patient only. Honest about the format's limit, same stance as v0.1's SBML notes. (`distrib`-package uncertainty objects are a possible future refinement; the RDF carrier is lossless and tool-portable today.) | ✅ |
...
| **V2 — Uncertainty-aware divergence** | ✅ done | separation index + variance decomposition in `compare()`; the headline figure (`docs/images/variability.png`); the **dashboard renders the bands as shaded Altair ribbons** with the separation index + variance-decomposition readout and the named band-excluded models (smoke-tested via Streamlit `AppTest`). | The divergence view answers "are the models distinguishable?" and "what dominates the uncertainty?" |
