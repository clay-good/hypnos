...
| **VE3 — Envelope stratification** | 🟢 shipped (stratification + report); tier-falsification flag + VPC proposed | each model re-scored on its in-/out-of-envelope sub-cohorts (`partition_by_envelope` via `Envelope.check`), carried in the leaderboard record + the `--report` markdown — turning v0.1's asserted failure modes into measured ones. **Shipped:** the `validate` tier-mismatch flag (§5, advisory). **Shipped:** `visual_predictive_check` bins observed vs model-predicted percentiles (the VPC as reproducible data; the figure is a rendering). | The tiers are now empirically stratified and the report is a reproducible, citable artifact. |
