...
| **VE1 — PD/BIS validation (VitalDB)** | proposed | VitalDB adapter; PD-mode validation of the composed PK+link+BIS stack against recorded BIS; data-manifest + ethics handling (§3). | The depth-of-anesthesia stack is validated end-to-end on thousands of open cases, with no patient data committed. |
| **VE2 — PK-concentration validation (Open-TCI)** | proposed | Open-TCI propofol adapter; PK-mode concentration validation; the cross-model leaderboard on an identical cohort. | The gold-standard concentration validation runs; the open, reproducible propofol leaderboard exists. |
| **VE3 — Envelope stratification + tier falsification + VPC** | proposed | in/out-of-envelope stratified metrics; `known_failure_mode` confirmation rows; the `validate` tier-mismatch flag (§5); VPC overlays on v0.2 bands; the citable `--report` artifact. | The tiers become falsifiable and the failure modes become measured; the validation report is a reproducible, citable output. |
