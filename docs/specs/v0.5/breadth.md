...
| **S0 — Organ-function envelope** | proposed | extend `applicability_envelope` + `evaluate_safety`/`Envelope.check` to hepatic/renal/cardiac/albumin dimensions; the envelope-shrinkage overlay in `compare`. | An organ-failure virtual patient correctly greys out and Tier-D's the models with no standing. |
