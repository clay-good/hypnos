...
> **Status:** **S0 + S1-binding + the reversal subsystem (B0–B1) implemented; and the quantitative disease modifiers ship — the subsystem is COMPLETE.** The `reversal` subsystem ships: three forward-only kernels (`hypnos.reversal` — sugammadex encapsulation, naloxone/flumazenil competitive shift, neostigmine indirect inhibition), the `reversal_mechanism` schema block, `simulate_reversal` + `hypnos reverse`, the targets-resolve + renarcotization-caveat validator, and curated sugammadex/naloxone/neostigmine records. The canonical sugammadex→rocuronium→TOF pairing simulates end-to-end (TOF recovers ~1%→98% by 30 min); naloxone renders the renarcotization relapse forward; neostigmine carries the hard deep-block ceiling. No reversal dose is ever computed (§A6). The rocuronium PK kernel is now implemented from sourced micro-constants (pending_human_review). The physiological envelope speaks: `applicability_envelope` carries optional organ-function ranges + a cited `organ_tolerance[]` block, `Envelope.organ_check` / `evaluate_safety` grey every model with no standing in a declared hepatic/renal/cardiac/albumin impairment (Tier-D + named extrapolation), the remifentanil esterase exception is encoded with citations (Dershwitz 1996 / Hoke 1997). For **S1**, the **protein-binding / free-fraction failure mode** (§B3/§B7) is curated and surfaced: binding-sensitive drugs (propofol/fentanyl/dexmedetomidine) carry a cited drug-level `protein_binding` block, and a hypoalbuminemic patient triggers the cited `BINDING-SENSITIVE` caveat — the free-fraction shift named, not modeled. Live in `compare`, the CLI, and the dashboard. The **opt-in, Tier-D, quantitative** `disease_state_modifier` now ships too: a `disease_state_modifier` schema block + `simulate(..., disease_state=True)` applies a cited, declared-state adjustment (forcing Tier-D + a warning), with the never-invent rule enforced by `validate` (no adjustment without a citation). Curated: dexmedetomidine clearance reduced in hepatic impairment (De Wolf 2001). Additive over [v0.1](../v0.1/spec.md)–[v0.4](../v0.4/external_validation.md): no existing record changes meaning; both subsystems are new `subsystem`/`purpose` values plus optional blocks. Dataset/schema target `0.5.0`. This spec builds the two items v0.1 named and held back: the `reversal` subsystem (v0.1 §3 lists it; only `rocuronium_tof_pd` exists today, no reversal agent is curated) and disease-state PK, which v0.1 §2 explicitly placed out of v0.x scope as *"a future tier-D research envelope."* v0.5 designs that envelope so the extrapolation is *labeled and bounded*, never silent.
...
| --- | --- | --- | --- |
| **B0 — Sugammadex encapsulation** | 🟢 shipped | `reversal` subsystem + schema; sugammadex record + `encapsulation_ode` kernel reversing the existing rocuronium PK→TOF stack; renarcotization/recovery as forward output. | The canonical reversal pairing (rocuronium + sugammadex) simulates end-to-end, recovery rendered, no dose computed. |
| **B1 — Competitive & indirect antagonists** | 🟢 shipped | naloxone (opioid) + flumazenil (benzodiazepine) competitive-shift kernel; neostigmine indirect/ceiling kernel; renarcotization & deep-block-ceiling failure modes. | The three reversal mechanisms are each modeled and each carries its safety-relevant failure mode. |
...
| **S1 — Disease modifiers + binding** | 🟢 shipped | The **free-fraction / protein-binding failure mode** is curated (drug-level `protein_binding` for propofol/fentanyl/dexmedetomidine, cited) and surfaced as a `BINDING-SENSITIVE` caveat whenever a hypoalbuminemic patient meets a binding-sensitive drug — the binding story is surfaced, never modeled. **Remaining:** the opt-in, Tier-D, quantitative `disease_state_modifier` scale-factor records, which need per-drug source curation of the adjustment magnitude. | The binding story is surfaced (done); documented quantitative disease adjustments curated + applied only with a citation + warning (pending magnitude curation). |
