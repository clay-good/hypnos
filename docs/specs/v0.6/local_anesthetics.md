...
| **LA3 — Binding saturation failure mode** | 🟢 shipped | A `free_fraction_model` (`capacity_limited`) on bupivacaine/levobupivacaine/ropivacaine; the exact `la.saturable_free_concentration` Langmuir kernel (affinity pinned by the curated `fraction_bound`; capacity ≈ AAG molar conc × drug MW, cited, **Tier-D illustrative**); `la.free_concentration` returns the non-linear trace beside the linear baseline; the double-uncertainty view + CLI name the under-prediction gap; `hypnos validate`/`verify` checks; `hypnos:freeFractionModel` in every export; a `la_saturation.png` figure. | Total concentration's under-prediction of free-drug risk at high concentration is made visible, not hidden (free fraction climbs from ~5% toward 1 as binding saturates; the gap is named, Tier-D). |
