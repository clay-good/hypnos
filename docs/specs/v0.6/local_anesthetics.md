...
> **Status:** **LA0 shipped (disposition + site absorption + binding, NO thresholds); LA1–LA3 proposed.** The `local_anesthetics` subsystem is live: an additive `absorption` schema block (site rates with a robust `rank` + Tier-C `ka`), three curated LA models (lidocaine/bupivacaine/ropivacaine, Tucker & Mather 1979, `unverified`), the forward-only Bateman kernel + `site_comparison` in `hypnos.la`, the `hypnos la` CLI surfacing the **site-of-injection dominance**, and drug-level binding curated and correctly kept off the v0.5 albumin axis (LA binding is α1-AG-driven). Deliberately **no toxicity thresholds yet** — the safest possible entry point teaches the site-dominance idea before any threshold could be misread. Still proposed: the `toxicity_threshold` ranges + the double-uncertainty view (LA1), the stereochemistry/cardiotoxicity differentiation (LA2), and the binding-saturation free-fraction failure mode (LA3). Additive over [v0.1](../v0.1/spec.md)–[v0.5](../v0.5/breadth.md). Dataset/schema target `0.6.0`. This subsystem is the one v0.1 twice held back on purpose: v0.1 §2 lists `local_anesthetics` ⚠️ as **deferred** ("the safety framing needs its own pass before shipping"), and v0.1 §10 repeats *"local-anesthetic systemic-toxicity thresholds are deferred until the safety framing for that subsystem is designed separately."* This document **is** that separate safety-framing pass. It is written safety-first: the guardrails (§7) are not an appendix but the reason the subsystem exists in this shape.
...
| **LA1 — Toxicity thresholds + the double-uncertainty view** | proposed | `toxicity_threshold` ranges (CNS + cardiovascular) with basis/uncertainty; the double-uncertainty view (§6); the free-concentration trace; the schema range-not-line enforcement. | The honest concentration-vs-threshold-*band* view exists, framed as research/education, with the threshold uncertainty made explicit. |
| **LA2 — Stereochemistry / cardiotoxicity** | proposed | `cardiotoxicity_class` differentiation (racemic bupivacaine vs. levobupivacaine vs. ropivacaine); the agent-choice axis in the divergence view. | The view shows *why* agent choice changes the cardiovascular margin, the key educational comparison. |
