...
| **C1 — Equation-divergence view** | proposed | `covariate-divergence` CLI + `compare` over equations; greying equations outside their own envelope; the Schnider-obesity failure mode rendered at its covariate source; the headline figure (`docs/images/covariate_divergence.png`). | The "divergence within a model" view exists and shows the James inversion as the documented failure mode it is. |
| **C2 — Covariate-value bands + the fifth variance component** | proposed | `sample_covariate_vector`; the seeded covariate band in `simulate`; the never-invent rule for value distributions; the `covariate` share + `covariate_split` + refined `reducibility` in `compare`; dashboard readout. | `simulate(..., bands=[..., "covariate"], seed=...)` returns reproducible covariate quantiles; the decomposition separates equation-choice from value-uncertainty. |
