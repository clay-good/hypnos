...
| **C3 — Exports** | 🟢 shipped | the derived-covariate computation made explicit in NONMEM `$PK`, PharmML `Covariate` transforms, SBML `AssignmentRule`, TCI-JSON passthrough; round-trip equality of the exported computation vs. the library; `hypnos:` covariate RDF + envelope warnings. | The covariate transform round-trips through the pharmacometric formats; no export can silently recompute LBM. |
